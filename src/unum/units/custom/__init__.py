...
def generate_units(base_unit_name, base_unit_symbol, base_unit_description, prefixes):
    ...
    for prefix, (symbol_prefix, multiplier) in prefixes.items():
        ...
        units[unit_symbol] = unit(unit_symbol, multiplier * base_unit, unit_description)
...
MTON = mton = ton = unit('ton', kg * 1000, 'metric-ton')
Btu = btu = unit('Btu', J * 1055, 'british-thermal-unit')
MBtu = mbtu = unit('MBtu', Btu * 10**6, 'mega-btu')
